...
// =============================
// FUNÇÃO 4 — ANÁLISE GLOBAL (FASE DE VIDA)
// =============================
const GLOBAL_ANALYSIS_PROMPT = `Você é uma inteligência especializada em psicologia profunda e arquétipos junguianos.
Analise o histórico de sonhos do usuário e identifique:
1. Qual é o ARQUÉTIPO dominante neste momento da vida dele?
2. Qual é a "Fase de Vida" ou "Jornada" que ele está atravessando?
3. Quais são os desafios ocultos e as oportunidades de crescimento?

Retorne um JSON estrito com o seguinte formato:
{
  "archetype": "Nome do Arquétipo (ex: O Explorador, O Mago)",
  "phaseName": "Nome da Fase (ex: A Travessia do Deserto)",
  "description": "Texto explicativo profundo e acolhedor sobre esta fase e o arquétipo.",
  "keyChallenges": ["Desafio 1", "Desafio 2"],
  "opportunities": ["Oportunidade 1", "Oportunidade 2"]
}
Seja profundo, poético mas claro.`;

export const analyzeGlobalDreamsWithGPT5 = async (dreams, userId, language = "pt") => {
    try {
        console.log(`[OpenAI] analisando fase de vida (global) para ${userId}`);

        const dreamsSummary =
            dreams?.length
                ? dreams
                      .slice(0, 10) // Limita aos 10 últimos para não estourar contexto
                      .map((d) => `- ${d.dreamTitle || "Sonho"}: ${d.interpretationMain || ""}`)
                      .join("\n")
                : "Nenhum sonho recente.";
        ...
        const response = await client.responses.create({
            ...
            input: [
                {
                    role: "system",
                    content: [{ type: "input_text", text: GLOBAL_ANALYSIS_PROMPT }]
                },
                {
                    role: "user",
                    content: [
        ...
        console.error("[OpenAI ERROR globalAnalysis]", err);
        throw err;
    }
};
